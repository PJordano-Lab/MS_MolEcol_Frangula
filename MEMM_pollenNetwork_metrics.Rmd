...
params.JV <- params.net(paramDispfile = "data/MEMM_files/ParamDisp_JV.txt", file2 = "data/MEMM_files/IndivFec_JV.txt", file3 = "data/MEMM_files/JV_adult.txt")
params.SAUC <- params.net(paramDispfile = "data/MEMM_files/ParamDisp_SAUC.txt", file2 = "data/MEMM_files/IndivFec_SAUC.txt", file3 = "data/MEMM_files/SAUC_adult.txt")
params.PL <- params.net(paramDispfile = "data/MEMM_files/ParamDisp_PL.txt", file2 = "data/MEMM_files/IndivFec_PL.txt", file3 = "data/MEMM_files/PL_adult.txt")
params.ZP <- params.net(paramDispfile = "data/MEMM_files/ParamDisp_ZP.txt", file2 = "data/MEMM_files/IndivFec_ZP.txt", file3 = "data/MEMM_files/ZP_adult.txt")
...
# These are the actual adjacency mnatrices, based on median values, once filtered by `mat.JV[mat.JV < 0.05] <- 0`.
### JV pop ###

mat.JV <- pollenNetwork(file = "data/MEMM_files/ParamDisp_JV.txt", file2 = "data/MEMM_files/IndivFec_JV.txt", file3 = "data/MEMM_files/JV_adult.txt", file4 = "data/pollenNetwork_matrices/JV_pollenNetwork2", nloc=16, nql=0, nqt=0, nqw=0, forBreaks=c(0,0.05,0.1,0.5,1))
...
mat.SAUC <- pollenNetwork(file = "data/MEMM_files/ParamDisp_SAUC.txt", file2 = "data/MEMM_files/IndivFec_SAUC.txt", file3 = "data/MEMM_files/SAUC_adult.txt", file4 = "data/pollenNetwork_matrices/SAUC_pollenNetwork2", nloc=16, nql=0, nqt=0, nqw=0, forBreaks=c(0,0.05,0.1,0.5,1))
...
mat.PL <- pollenNetwork(file = "data/MEMM_files/ParamDisp_PL.txt", file2 = "data/MEMM_files/IndivFec_PL.txt", file3 = "data/MEMM_files/PL_adult.txt", file4 = "data/pollenNetwork_matrices/PL_pollenNetwork2", nloc=16, nql=0, nqt=0, nqw=0, forBreaks=c(0,0.05,0.1,0.5,1))
...
mat.ZP <- pollenNetwork(file = "data/MEMM_files/ParamDisp_ZP.txt", file2 = "data/MEMM_files/IndivFec_ZP.txt", file3 = "data/MEMM_files/ZP_adult.txt", file4 = "data/pollenNetwork_matrices/ZP_pollenNetwork2", nloc=16, nql=0, nqt=0, nqw=0, forBreaks=c(0,0.05,0.1,0.5,1))
...
Modularity estimates for the thresholded mating networks in each population.
```{r modularity}
...
# Modularity tests -- with our function(modularity)
# --------------------------------------------------------------------
# Modularity function
...
mymat <- mat.JV
modularity(mymat)
modularity(pre2007[,4:102])
modularity(apis2007[,4:102])
modularity(pre2008[,4:102])
modularity(apis2008[,4:102])
modularity(subpre2009[,4:102])
modularity(apis2009[,4:102])
modularity(pre2008sub2009)
modularity(apis20082009)
#
# Testing modularity significance (@30 min for 100 null replicates)
require(bipartite)
TIME <- Sys.time()
mat <- pre2007[,4:102]     # Assign the observed matrix
...
# --------------------------------------------------------------------
Msig(pre2007[,4:102], M_pre2007)
Msig(apis2007[,4:102], M_apis2007)
Msig(pre2008[,4:102], M_pre2008)
Msig(apis2008[,4:102], M_apis2008)
Msig(subpre2009[,4:102], M_presub2009)
Msig(apis2009[,4:102], M_apis2009)
Msig(pre2008sub2009, M_pre2008sub2009)
Msig(apis20082009, M_apis20082009)
# --------------------------------------------------------------------
Sys.time() - TIME 
# --------------------------------------------------------------------
# Testing modularity significance (@30 min for 100 null replicates)
TIME <- Sys.time()
mat <- newpre2007[,4:102]     # Assign the observed matrix
res <- computeModules(mat) # Modularity for observed matrix
nulls <- nullmodel(mat, N=100, method=3)
modules.nulls <- sapply(nulls, computeModules)
like.nulls <- sapply(modules.nulls, function(x) x@likelihood)
z <- (res@likelihood - mean(like.nulls))/sd(like.nulls)
p <- 2*pnorm(-abs(z))
cat(formatC(p, digits = 8, format = "e")," ")
Sys.time() - TIME 
#
# Batch to generate the null models for modularity M significance test. ----- 
# (for each matrix, @30 min for 100 null replicates)
...
# Give here the mean observed modularity values.
    ...
    M_pre2008 <- 0.37325
    M_apis2008 <- 0.43367
    M_presub2009 <- 0.31420
    M_apis2009 <- 0.47407
    M_pre2008sub2009 <- 0.31749
    M_apis20082009 <- 0.46129
Msig <- function (mat, mlike)  {
    # mat is the input matrix for which M is tested
    # mlike is the observed mean M value
...
## TESTING FOR DIFFERENCES AMONG POPULATIONS
