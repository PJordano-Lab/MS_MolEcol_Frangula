...
# TIME <- Sys.time()
# mat <- mymat    # Assign the observed matrix
...
# Give here the mean observed modularity values.
    M.JV <- 0.69005 # 9 modules
    M.SAUC <- 0.69737 # 18 modules
    M.PL <- 0.79659 # 10 modules
    M.ZP <- 0.78765 # 11 modules
...
# ANALYSIS OF ASSORTATIVITY IN MATING
...
assort <- function(mat) {
  library(igraph)
    ...
    weighted = TRUE,
    diag = FALSE,
    add.colnames = "mat.tree",
    add.rownames = "pat.tree"
  )
...
### Correlates network parameters

```{r netwk_corr}
netwkparams<- read.table("data/network_params.txt", header=T, sep="\t", dec=".", 
                         na.strings="NA")

cor(netwkparams[,2:8], use = "everything",
    method = "kendall")
